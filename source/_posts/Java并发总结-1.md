...
想要避免死锁, 只需要**破坏至少一个构成死锁的必要条件即可!** <red>但是, 由操作系统可知, 只有`请求并持有`和`环路等待`条件是可以被破坏的!</font>

-   **保证资源申请的有序性**

造成死锁的原因其实和<green>申请资源的顺序</font>有很大关系. 使用资源申请的有序性即可!
...
**原因**: <red>资源的有序性破坏了资源请求并持有条件和环路等待条件!</font>
...
<green>当最后一个`非守护线程`退出时, JVM将会退出, 所有的守护线程也会退出!</font>



#### 2): 守护线程的创建

在Java中创建守护线程只需: <green>设置线程的daemon参数为`true`即可!</font>
...
通过源码可以看出: <red>在默认情况下, *接受线程和处理线程都是守护线程!*</font>

这也意味着: <red>当tomcat收到shutdown命令后, 并且没有其他用户线程存在时, Tomcat进程将会马上消亡, *不会等待处理线程处理完当前请求!</font>
